import { ChangeDetectionStrategy, Component, ContentChildren, Directive, ElementRef, EventEmitter, Inject, Injectable, InjectionToken, Input, NgModule, NgZone, Optional, Output, ViewChild, ViewContainerRef, ViewEncapsulation } from '@angular/core';
 ...
 */
const loadScript = (url, scriptId, window, loadedObjKey = 'loadedScripts') => {
 ...
 * @nocollapse
 */
MapLoadedService.ctorParameters = () => [];
...
class OptionBuilder {
     ...
     */
    getMarkerIcon(input) {
        let /** @type {?} */ output = Object.assign({}, input);
...
class MapService {
     ...
     * @return {?}
     */
    setObjectEvents(definedEvents, componentInstance, mapObject) {
        definedEvents.forEach((definedEvent) => {
            const /** @type {?} */ eventName = this.normalizeEventName(definedEvent);
            const /** @type {?} */ zone = this.zone;
            zone.runOutsideAngular(() => {
                mapObject.addListener(eventName, function (event) {
                    let /** @type {?} */ param = event ? event : {};
                    // param.target = this;
                    zone.run(() => componentInstance[definedEvent].emit(param));
                });
            });
        });
    }
     ...
     * @return {?}
     */
    normalizeEventName(definedEvent) {
        return definedEvent
...
class MarkerClustererProviderService {
    constructor() {
    ...
    getClusterer() {
        if (!this.MarkerClusterer$) {
            this.MarkerClusterer$ = loadScript(this.clustererLibUrl, 'marker-clusterer', window)
                .pipe(map(() => MarkerClusterer), publishReplay(1), refCount());
        }
 ...
 * @nocollapse
 */
MarkerClustererProviderService.ctorParameters = () => [];
    ...
    return class extends OverlayView {
        ...
        constructor(...args) {
            ...
            this.setPosition = (position) => {
                ...
                const /** @type {?} */ projection = this.getProjection();
                if (projection) {
                    let /** @type {?} */ posPixel = projection.fromLatLngToDivPixel(this._position);
         ...
         */
        _initDraggable() {
            const /** @type {?} */ addDomListener = google.maps.event.addDomListener;
            this._htmlElement.draggable = true;
            this._mouseLeaveMapListener = addDomListener(this.mapElement, 'mouseleave', () => this._dragOrigin && this._onDragEnd());
            // TODO: check if we can use event as parameter
            this._mouseDownListerer = addDomListener(this._htmlElement, 'mousedown', (event) => {
                this._map.set('draggable', false);
                this._dragOrigin = event;
                this._moveHandler = addDomListener(this.mapElement, 'mousemove', (event) => {
                    const /** @type {?} */ origin = this._dragOrigin;
                    if (!origin) {
                        return;
                    }
                    const /** @type {?} */ leftOffset = origin.clientX - event.clientX;
                    const /** @type {?} */ topOffset = origin.clientY - event.clientY;
                    const /** @type {?} */ currentPositionPx = this.getProjection()
                        .fromLatLngToDivPixel(this._position);
                    const /** @type {?} */ nextLatLng = this.getProjection()
            ...
            this._mouseUpListener = addDomListener(this._htmlElement, 'mouseup', () => this._dragOrigin && this._onDragEnd());
        }
        /**
         * @return {?}
         */
...
class CustomMarkerOverlayViewService {
    /**
     * @param {?} _mapLoadedService
     */
    constructor(_mapLoadedService) {
        this._mapLoadedService = _mapLoadedService;
        const _overlayDefined$ = new ReplaySubject();
        this.overlayDefined$ = _overlayDefined$.asObservable();
        _mapLoadedService.loaded$
            .pipe(filter$2(Boolean), first())
            .subscribe(() => {
            this.CustomMarkerOverlayView = CustomMarkerOverlayView(google.maps.OverlayView);
 ...
 * @nocollapse
 */
CustomMarkerOverlayViewService.ctorParameters = () => [
...
class NguiMapComponent {
     ...
     * @param {?} _mapLoadedService
     */
    constructor(optionBuilder, elementRef, geolocation, geoCoder, nguiMap, apiLoader, zone, _mapLoadedService) {
        this.optionBuilder = optionBuilder;
    ...
    initializeMap() {
        ...
        this.zone.runOutsideAngular(() => {
            ...
            // set google events listeners and emits to this outputs listeners
            this.nguiMap.setObjectEvents(OUTPUTS$1, this, this.map);
...
class Circle extends BaseMapDirective {
     ...
     */
    constructor(nguiMapComp) {
        super(nguiMapComp, 'Circle', INPUTS$3, OUTPUTS$3);
...
const INPUTS$4 = ['controlPosition', 'controls', 'drawingMode', 'featureFactory', 'style', 'geoJson', 'geoJsonUrl'];
const OUTPUTS$4 = [
...
class DataLayer extends BaseMapDirective {
    ...
    constructor(nguiMapComponent) {
        super(nguiMapComponent, 'Data', INPUTS$4, OUTPUTS$4);
    }
    /**
     * @return {?}
     */
...
const OUTPUTS$5 = ['directions_changed'];
class DirectionsRenderer extends BaseMapDirective {
    ...
    constructor(nguiMapComponent, geolocation) {
        super(nguiMapComponent, 'DirectionsRenderer', INPUTS$5, OUTPUTS$5);
        this.geolocation = geolocation;
    }
    /**
     * @return {?}
...
class DrawingManager extends BaseMapDirective {
     ...
     */
    constructor(nguiMapComp) {
        super(nguiMapComp, 'DrawingManager', INPUTS$6, OUTPUTS$6);
...
const INPUTS$7 = ['url', 'bounds', 'clickable', 'opacity'];
const OUTPUTS$7 = ['click', 'dblclick'];
class GroundOverlay extends BaseMapDirective {
    /**
     * @param {?} nguiMapComp
     */
    constructor(nguiMapComp) {
        super(nguiMapComp, 'GroundOverlay', INPUTS$7, OUTPUTS$7);
        this.objectOptions = ({});
    }
    /**
     * @return {?}
...
const INPUTS$8 = ['data', 'dissipating', 'gradient', 'maxIntensity', 'opacity', 'radius', 'options'];
const OUTPUTS$8 = [];
class HeatmapLayer extends BaseMapDirective {
     ...
     */
    constructor(nguiMapComp) {
        super(nguiMapComp, 'HeatmapLayer', INPUTS$8, OUTPUTS$8);
...
const INPUTS$9 = ['clickable', 'preserveViewport', 'screenOverlays', 'suppressInfoWindows', 'url', 'zIndex', 'options'];
const OUTPUTS$9 = ['click', 'defaultviewport_changed', 'status_changed'];
class KmlLayer extends BaseMapDirective {
    /**
     * @param {?} nguiMapComp
     */
    constructor(nguiMapComp) {
        super(nguiMapComp, 'KmlLayer', INPUTS$9, OUTPUTS$9);
    }
}
KmlLayer.decorators = [
    { type: Directive, args: [{
...
class Marker extends BaseMapDirective {
     ...
     */
    constructor(nguiMapComp) {
        super(nguiMapComp, 'Marker', INPUTS$10, OUTPUTS$10);
        this.nguiMapComp = nguiMapComp;
        this.objectOptions = ({});
    }
    /**
...
class Polygon extends BaseMapDirective {
     ...
     */
    constructor(nguiMapComp) {
        super(nguiMapComp, 'Polygon', INPUTS$11, OUTPUTS$11);
    }
}
Polygon.decorators = [
    { type: Directive, args: [{
...
class Polyline extends BaseMapDirective {
     ...
     */
    constructor(nguiMapComp) {
        super(nguiMapComp, 'Polyline', INPUTS$12, OUTPUTS$12);
    }
}
Polyline.decorators = [
    { type: Directive, args: [{
...
class StreetViewPanorama extends BaseMapDirective {
     ...
     */
    constructor(nguiMapComp) {
        super(nguiMapComp, 'StreetViewPanorama', INPUTS$13, OUTPUTS$13);
    }
    /**
     * @return {?}
     */
...
const INPUTS$14 = ['autoRefresh', 'options'];
const OUTPUTS$14 = [];
class TrafficLayer extends BaseMapDirective {
     ...
     */
    constructor(nguiMapComp) {
        super(nguiMapComp, 'TrafficLayer', INPUTS$14, OUTPUTS$14);
    }
}
...
class TransitLayer extends BaseMapDirective {
     ...
     */
    constructor(nguiMapComp) {
        super(nguiMapComp, 'TransitLayer', INPUTS$15, OUTPUTS$15);
    }
}
...
class CustomMarkerComponent {
     ...
     * @param {?} _mapService
     */
    constructor(_customMarkerOverlayViewService, _nguiMapComponent, _elementRef, _mapService) {
        ...
        OUTPUTS$16.forEach(output => this[output] = new EventEmitter());
    }
    /**
     * @return {?}
     */
 ...
 * @nocollapse
 */
CustomMarkerComponent.ctorParameters = () => [
...
CustomMarkerComponent.propDecorators = {
    ...
    'position': [{ type: Input },],
    'attachToParentMap': [{ type: Input },],
};

const getDefaultImageInline = (color = '#004b7a') => `
    <svg
...
class CustomMarkerClusterDirective {
    /**
     ...
     * @param {?} _nguiMapComponent
     */
    constructor(_markerClustererProviderService, _mapLoadedService, _mapService, _nguiMapComponent) {
        ...
        outputs.forEach((eventName) => this[eventName] = new EventEmitter());
    }
    /**
     * Default styles are: {
     *     width: 40,
     ...
     * @return {?}
     */
    set clusterStyles(clusterStyles) {
     ...
     * @return {?}
     */
    set customMarkers(customMarkers) {
    ...
    _updateMarkers() {
        (this._customMarkers.length === 0
            ? of([])
            : combineLatest(...this._customMarkers.map((customMarkerCmp) => customMarkerCmp.intialized$)))
            .subscribe((overlays) => {
     ...
     * @return {?}
     */
    _getGoogleClusterInlineSvg() {
        const /** @type {?} */ encoded = window.btoa(this.rawImage || getDefaultImageInline(this.color));
 ...
 * @nocollapse
 */
CustomMarkerClusterDirective.ctorParameters = () => [
...
CustomMarkerClusterDirective.propDecorators = {
    'clusterStyles': [{ type: Input },],
    'rawImage': [{ type: Input },],
    'color': [{ type: Input },],
    'options': [{ type: Input },],
    'customMarkers': [{ type: ContentChildren, args: [CustomMarkerComponent,] },],
};
...
export { BicyclingLayer, NavigatorGeolocation, OptionBuilder, NG_MAP_CONFIG_TOKEN, NgMapApiLoader, NgMapAsyncApiLoader, NguiMapComponent, InfoWindow, Circle, DataLayer, DirectionsRenderer, DrawingManager, GeoCoder, GroundOverlay, HeatmapLayer, KmlLayer, Marker, MapService, PlacesAutoComplete, Polygon, Polyline, StreetViewPanorama, TrafficLayer, TransitLayer, NguiMapModule, CustomMarkerComponent as ɵf, CustomMarkerOverlayViewService as ɵc, BaseMapDirective as ɵa, CustomMarkerClusterDirective as ɵe, MapLoadedService as ɵb, MarkerClustererProviderService as ɵd };
